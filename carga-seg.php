...
<div class="wrapper">
	<div class="widget">
	<form action="carga-seg.php" method="POST" enctype="multipart/form-data">
		<fieldset>
				...
				while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
					$num = count($data);
					 for ($c=0; $c < $num; $c++) {
						...
						$fecha=trim($tmp[3]);
						$evento=trim(strtoupper($tmp[2]));
						$trata=trim(strtoupper($tmp[4]));
						...
						if(mysql_num_rows($result)>0){
							...
							if($row['ID']!=null&&$row['ID']!=""){
								...
								if($fecha!=""){
									...
									//$anti=trim(@$tmp[4]);
									$obs=trim(@$tmp[5]);
									//$enf=strtoupper(trim(@$tmp[2]));
									$query = "SELECT ID from PacienteEvento WHERE PacienteID=$pid AND NEvento=$control;";
									$result = mysql_query($query);
									/*if($cv==""){
										$cv="NULL";
									}*/
									...
									/*$query="SELECT ID from Patologia WHERE Codigo='$enf';";
									$result2=mysql_query($query);
									if(mysql_num_rows($result2)==1){	
										$row = mysql_fetch_array($result2);									
										$enf=$row["ID"];*/
										if(mysql_num_rows($result)==0){
											echo "<td style=\"background-color:red;color:white;\">INSERT INTO PacienteEvento (PacienteID,EventoID,NEvento,Fecha,Tratamiento,Observacion) ";
											echo " VALUES ($pid,$evento,$control,'$fecha','$trata','$obs');</td>";
											
										//	echo "<td style=\"background-color:red;color:white;\">UPDATE Control SET Fecha='$fecha' Where PacienteID=$pid AND Tipo='$t' AND NumeroControl=$control and Fecha='0000-00-00';</td>";
										}
									//}
								}
							}else{
